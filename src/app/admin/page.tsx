            ...
            if (attributionData.length === 0) {
              return (
                <div className="py-6 text-center">
                  <p className="font-(family-name:--font-inter) text-white/50">No attribution traffic logged yet.</p>
                </div>
              );
            }
                ...
                <table className="w-full text-left text-sm text-white/80">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {attributionData.map((data) => (
                      <tr key={data.source} className="hover:bg-white/[0.01] transition-colors">
                        <td className="py-3 font-medium text-amber-400 capitalize">{data.source}</td>
                        <td className="py-3 text-center font-semibold">{data.clicks}</td>
                        <td className="py-3 text-center font-semibold">{data.signups}</td>
                        <td className="py-3 text-right font-semibold text-emerald-400">
                          {data.clicks > 0 ? `${((data.signups / data.clicks) * 100).toFixed(1)}%` : "0.0%"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
